...
class IOAOptionsPanel extends IOA_PANEL_CORE {
	...
	function manager_admin_init(){	
	   global $ioa_options , $ioa_super_options,$ioa_helper;
	   ...
	   if(isset($_GET['ioa_reset']))
	   	{
	   		setDemoOptions();
	   		$ioa_helper->preIOACacheSettings(); // Cache options again
	   	}
	...
	function panelmarkup(){	
		...
		global $themename,  $ioa_options , $ioa_super_options;
        ...
        <div class=" clearfix">  <!-- Panel -->
        	<div class="option-panel-top-bar clearfix">
        			<div class="ioa_branding_text">
						<?php 
							if(trim($ioa_super_options[SN.'_cbrand_text'])!="") 
								echo "<p id='cbrand_text'>".$ioa_super_options[SN.'_cbrand_text']."</p>"; 
        	...
        	<div id="option-panel-tabs" class="clearfix option-panel-tabs fullscreen"  data-shortname="<?php echo SN; ?>">  <!-- Options Panel -->
				...
				<?php if($ioa_super_options[SN.'_cbrand_toggle'] == "true") : ?>
				
				<?php if($ioa_super_options[SN.'_cbrand_logo']!="") echo "<img id='cbrand_logo' src='".$ioa_super_options[SN.'_cbrand_logo']."' alt='admin-branding'/>" ?>		
 			  		 ...
 			  		 switch ( $value['type'] ) {
                		 ...
                		 case 'custom' : echo ($value['std']);   break;	
                   		 case 'text':	
                   		 		...
                   		 		if(!isset($value['addMarkup'])) $value['addMarkup'] = '';
                   		 		echo getIOAInput( 
							array( 
									"label" => $value['name'] , 
									"class" => $value['class'] , 
									"addMarkup" => $value['addMarkup'] , 
									"name" => $value['id'], 
									"default" => '', 
									"type" => "text",
									"description" => $value['desc'],
									"length" => 'medium',
									"value" => $ioa_super_options[$value['id']]  
							) 
						);
						break;
                
		 	case 'header_module' : $h = new IOAHeaderConstructor(); $h->getHeadBuilder(); break;
		 	case 'visual_module' :  $visual->panelmarkup(); break;
		 	case 'fontface_module' :  $visual->fontFacePanel(); break;
		 	case 'fontdeck_module' :  $visual->fontDeckPanel(); break;
		 	case 'adv_visual_module' :  $visual->advModule(); break;
		 	case 'installer_module' : $i = new IOAInstaller(); $i->panelmarkup(); break;
		 	case 'power_module' : getPowerModule(); break;
		 	case 'testing_module' : getTestingModule(); break;
		 	case 'typography' :  $visual->typography(); break;
		 	case 'customcss' : $visual->customCSS(); break;
