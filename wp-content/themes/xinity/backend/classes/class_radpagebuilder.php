...
class RADPageBuilder
	{
		...
		static function canvasArea()
		{
			...
			<div class="rad-builder-area" data-id='<?php echo $post->ID; ?>'>
				...
				foreach ($data as $key => $section) {
					...
					RADMarkup::generateRADSection($d,$id,$containers);
					 $data = array();
		...
		{
			global $radunits,$post,$ioa_super_options;
...
function ioa_page_builder_lg()
{
	...
	<div class="rad_toolbox clearfix">
			...
			<ul  class="rad-swift-menu clearfix">
				...
				<li class='rad_pre_sections rad_image_prop parent_tip'><a href="#rad_pre_sections" class=""><img src='<?php echo HURL.'/css/i/section_icon.png' ?>' /><span>Sections</span><i class="ioa-front-icon left-dir-1icon-"></i><small class="rad_tooltip rleft">Import Inbuilt Section</small></a></li>
			</ul>
		 	 ...
		 	 <div id="rad_pre_sections" class="clearfix rad-widget-area">
		 	 		
		 	 		<div class="rad-widgets clearfix">

		 	 		<ul class='clearfix'>
						<li><a href="#rad_inbuilt_section">Predefined Sections</a></li>
						<li><a href="#rad_save_section">Saved Sections</a></li>
					</ul>

					<div class="clearfix rad-w-tab-content rad_tab_scroll" id="rad_inbuilt_section">
					<div class="rad-section-search-file clearfix">
						 <input type="text" class='rad-section-file-input' placeholder="<?php _e('Enter Section to Search','ioa') ?>" />
						 <i class="ioa-front-icon search-2icon-"></i>
					</div>

						<?php 
							$ins_path =   get_template_directory()."/sprites/rad_sections";
							if(file_exists($ins_path)) :
							$insta_templates = scandir($ins_path);
									$i = 0;
									$depth = 0;
									foreach ($insta_templates as $key => $template) {
										...
										{	
											...
											$name = str_replace("_"," ", $id);
											?>
											<div class="insta-section-template rad-template-icon insta-section-template-trigger <?php if($i%3==0) {  echo 'first '; $depth++; } echo ' level-'.$depth;  ?>" data-source="<?php echo $template; ?>"> 
												<i class="ioa-front-icon list-alticon- file-icon "></i>
												<span class="label"><?php echo $name; ?></span> 
											</div>
											<?php
											$i++;
							...
							<div class="clearfix rad-w-tab-content rad_tab_scroll" id='rad_save_section'>
							<div class="ioa-information-p">
							<?php _e('Click on the Section to import it','ioa'); ?>
						</div>
							<?php 
							...
							foreach ($data as $key => $template) {
							  	...
							  	?>
							  	<div class="import-s-template rad-template-icon <?php if($i%3==0) {  echo 'first '; $depth++; } echo ' level-'.$depth;  ?>" data-key="<?php echo $key; ?>" data-source="<?php echo $template['title']; ?>"> 
												<i class="ioa-front-icon list-alticon-  file-icon"></i>
												<i class="ioa-front-icon cancel-3icon- delete-icon"></i>
												<span class="label"><?php echo $template['title']; ?></span> 
								</div>
