 ...
 if(!class_exists('IOACustomPostType'))
{
	class IOACustomPostType
	{
		...
		public function getHTML($values = false)
		{
			...
			if(isset($values['meta_name'])) $markup .= "<span>".$values['meta_name']."</span><span class='use'> ".__('To use this field , use this shortcode','ioa')." <strong>[get field='".$values['meta_name']."'/]</strong></span>";
...
{
	global $ioa_custom_posts;
	$ioa_custom_posts['default'] = new IOACustomPostType($inputs,$ioa_options);
