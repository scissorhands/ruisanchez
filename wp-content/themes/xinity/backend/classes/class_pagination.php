...
if (!class_exists('IOAWPPaginate')) {
	...
	function paginate_dropdown($args = array())
	{
		if ($this->type === 'comments' && !get_option('page_comments'))
			...
			if ($pages > 1) {
				$output .= '<div class="pagination-dropdown"><span>'.__('Goto','ioa').' :</span><div class="select-wrap"><i class="ioa-front-icon down-dir-1icon-"></i><select class="clearfix">';
...
function wp_paginate_dropdown($args = false) {
	global $wp_paginate,$ioa_super_options;

	if($ioa_super_options[SN."_pagination_dropdown"]!="true") return;
