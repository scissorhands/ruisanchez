...
class ioa_Menu_Frontend extends Walker_Nav_Menu  {
	...
	function start_lvl(&$output, $depth =0, $args = array()) {
		...
		{
			$element = 'div';
			$df = 'skeleton auto_align';
		}
		$output .= "\n$indent<{$element} class=\"sub-menu $df clearfix \"> \n";
	}
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if($menuicon!="") $menuicon = "<i class='icon $menuicon ioa-menu-icon'></i>";  
		...
		$item_output .= '<a'. $attributes .'><span class="m-el"> '.$menuicon.' ';
		$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
		
		if(trim($hovercolor)!="")
		$item_output .= ' </span><span class="color-element" style="background-color:'.$hovercolor.'">';
		$item_output .= ' </span><span class="hover-element">';
		$item_output .= ' </span></a> ';
		...
		if($depth==1)
		{
			...
			if($value_sidebar!="none")
			{
				...
				ob_start();
			      ...
			      $str = ob_get_contents();
			    ob_end_clean();
			    $item_output .= "<div class='sidebar menu-sidebar'>".$str."</div>";
			}
		}
		else
		$item_output .= '</a>';
...
class ioa_MegaMenu_Nav_Menu extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$value = $valueh =  $type = $textbox = $newrow = $menuicon = $value_sidebar = '';
			...
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<span class="item-type item-type-hmenu"><?php _e('(Mega Menu)','ioa'); ?></span>
				...
				<div class="menu-item-actions description-wide submitbox">
                   ...
                   <label for="menu-item-megamenu-<?php echo $item_id; ?>"><?php _e('Select Dropdown Type :','ioa') ?> </label>
					...
					<p class="ioa_megamenu_sidebar_box clearfix <?php if($value!="widget") echo 'hide'; ?>">
                    	<label for="menu-item-megamenu-sidebar-<?php echo $item_id; ?>"><?php _e('Select Widget Area:','ioa') ?> </label>
						...
						<?php 
						$osidebars = array("none" => "None", "Blog Sidebar");
...
add_action('admin_enqueue_scripts', 'hmenu_scripts');
				
add_action( 'wp_update_nav_menu_item', 'update_menu', 100, 3);
function update_menu($menu_id, $menu_item_db)
	{	
		 
		 $ioa_menu_worker = array( 'menuicon' => '', 'megamenu' => '' , 'sidebar' => '' ,  'megamenu-newrow' => '' , 'megamenu-layout' => '' , 'textbox' => '' );
