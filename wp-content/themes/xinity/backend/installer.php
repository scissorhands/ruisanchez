...
function setMetaData()
{
	...
	$wp_query = new WP_Query("post_type=page&posts_per_page=-1");
	 ...
	 while ( $wp_query->have_posts() ) : $wp_query->the_post();
	  ...
	  if(is_array($data))
 	  ...
 	  $data = preg_replace($pattern,addslashes($fn_i[0][0]),$data);
	...
	if(isset($data['ioa_titlearea_bgimage']) && $data['ioa_titlearea_bgimage']!="") $data['ioa_titlearea_bgimage'] = $fn_i[rand(0,2)][0];	  
	if(isset($data['video_fallback']) && $data['video_fallback']!="") $data['video_fallback'] = $fn_i[rand(0,2)][0];	  
...
public function importREVSliderFromPost($path){
			...
			try{
					...
					if(isset($sliderParams["background_image"]))
						$sliderParams["background_image"] =  "http://demo1.artillegence.com/xinity/wp-content/uploads/sites/4/".$sliderParams["background_image"];
					...
					foreach($arrSlides as $slide){
						...
						if(isset($params["image"]))
							$params["image"] =  "http://demo1.artillegence.com/xinity/wp-content/uploads/sites/4/".$params["image"];
						
						//convert layers images:
						foreach($layers as $key=>$layer){					
							if(isset($layer["image_url"])){
								$layer["image_url"] = "http://demo1.artillegence.com/xinity/wp-content/uploads/sites/4/".$layer["image_url"];
								$layers[$key] = $layer;
							}
						}
...
function setShopData()
{
	...
	$pay = get_page_by_title('Checkout &#8594; Pay');
	$thanks = get_page_by_title('Order Received');
	$my_account = get_page_by_title('My Account');
	$edit_address = get_page_by_title('Edit My Address');
	$change_password = get_page_by_title('Change Password');
	$lost_password = get_page_by_title('Lost Password');
	...
	update_option('woocommerce_pay_page_id',$pay->ID);
	update_option('woocommerce_thanks_page_id',$thanks->ID);
	update_option('woocommerce_myaccount_page_id',$my_account->ID);
	update_option('woocommerce_edit_address_page_id',$edit_address->ID);
	update_option('woocommerce_change_password_page_id',$change_password->ID);
	update_option('woocommerce_lost_password_page_id',$lost_password->ID);
