...
$post_structure_class = 'post-grid';
    ...
    <?php endif; ?>
       ...
       <?php
        if(isset($w['filter_menu']) && $w['filter_menu'] != "no") :
            get_template_part('templates/product-filter');
        endif;  
       ?>
    </div>
    
    <ul itemscope itemtype="http://schema.org/ItemList" data-gutter="50" data-layout="masonry" class="isotope hoverable no-canvas posts-grid posts  <?php echo $post_structure_class; ?>"  >          
    <?php query_posts($opts); $ioa_meta_data['i']=0; while (have_posts()) : the_post();  
         get_template_part('templates/product-grid-cols'); 
     endwhile; ?>
   </ul>
