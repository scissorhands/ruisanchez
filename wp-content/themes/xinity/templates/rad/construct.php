...
global $ioa_helper, $ioa_super_options , $ioa_meta_data ,$radunits, $post;

if( ! isset($ioa_meta_data['rad_id'])) $ioa_meta_data['rad_id'] = 1; // if template ID not set point to Home Page
...
{
	 $s = $data[0];
	 if(count($s['containers']) === 0 ) return;
}

 ?>

<div class="rad-holder clearfix" data-id="<?php echo $post->ID; ?>" >
...
<div class="page-section clearfix <?php echo join(' ',$cl) ?>" id='<?php echo $section['id']; ?>'>
	...
	<div class="<?php if($d['layout']!="Full Width") echo 'skeleton';  if($d['layout']=="Full Width") echo 'full_width'; if($d['layout']=="Cell Blocks") { echo " blocked-layout"; $ioa_meta_data['blocked'] = true; } ?> section-content auto_align clearfix">
		...
		<?php 
			...
			foreach ($section['containers'] as $key => $container) {
					$c = $container['data'];
					$c = $ioa_helper->getAssocMap($c,'value');
					$v = 0;
				...
				<div class="rad-container  layout_element <?php echo $float.' '.$container['layout'].$islast.$isfirst.$istop; if($c['background_opts']!='' && trim($c['background_color'])!="") echo ' senseBGModel '; if($c['border_top_width']!="" && $c['border_top_width']!="0") echo ' senseTopBorderModel '; if($c['border_bottom_width']!="" && $c['border_bottom_width']!="0") echo ' senseBottomBorderModel '; ?> clearfix" id="<?php echo $container['id'] ?>">
					...
					foreach ($widgets as $key => $widget) {
						 ...
						 {
						 	$widget['data'] = $ioa_helper->getAssocMap($widget['data'],'value');
						 }
