					...
					<?php if(isset($gallery_images) && trim($gallery_images) != "" ) : $ar = explode(";",stripslashes($gallery_images));
						...
						foreach( $ar as $image) :
						 ...
						 <div class="slider-item" data-thumbnail="<?php if(isset($g_opts[1])) echo $g_opts[1]; ?>">
                      		...
                      		{
                            if(isset($ioa_meta_data['adaptive_height']) && $ioa_meta_data['adaptive_height']=="true")
                              echo $ioa_helper->imageDisplay(array( "src" =>$g_opts[0] , 'imageAttr' =>  $g_opts[2], "parent_wrap" => false , "width" => $ioa_meta_data['width'] , "height" => $ioa_meta_data['height'], 'crop' => 'wproportional' )); 
                            else  
